...
| 2026-06 | Living World foundation (`tactical/living_world/`) added and made Godot-ready: **LivingWorldState** frozen in [`ENGINE_INTERFACES.md`](ENGINE_INTERFACES.md) and persisted additively under `WorldState.living_world` via `persistence.py` (round-trips through JSON; unknown keys ignored; legacy saves default safely). Region *content* is data (`RegionContent` manifests + `data/*.json`) consumed by a region-agnostic overlay engine — Godot reads the same JSON, no engine code per region. Core is screen-I/O-free (guarded by `region_review` + a test). CI gate: `scripts/ci_quality_gate.py` (headless, no Godot dependency). |
